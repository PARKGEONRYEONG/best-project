...
document.getElementById('loginBtn').addEventListener('click', async () => {
  ...
  if (signInError) {
    ...
    if (signUpError) {
      ...
      alert("회원가입 성공! 이제 로그인 버튼을 다시 눌러주세요.");
    }
  } else {
    alert("로그인 성공!");
    checkUser();
...
async function checkUser() {
  ...
  if (session) {
    ...
    fetchQuote();
    // 날짜 선택기 값이 있으면 그 날짜, 없으면 오늘 날짜로 불러오기
    const datePicker = document.getElementById('datePicker');
    fetchGoalsByDate(datePicker.value || new Date().toISOString().split('T')[0]);
  } else {
...
function renderGoals() {
  ...
  new Sortable(goalsContainer, { animation: 150, onEnd: async (evt) => {
      const movedItem = allGoals.splice(evt.oldIndex, 1)[0];
      allGoals.splice(evt.newIndex, 0, movedItem);
      for (let i = 0; i < allGoals.length; i++) { await supabase.from('goals').update({ order_index: i }).eq('id', allGoals[i].id); }
  }});
}
...
async function toggleGoalStatus(goal) {
  ...
  if (!error) { allGoals = allGoals.map(g => g.id === goal.id ? { ...g, is_active: !g.is_active } : g); renderGoals(); }
}

async function deleteGoal(e, goalId) {
  e.stopPropagation();
  const { error } = await supabase.from('goals').delete().eq('id', goalId);
  if (!error) { allGoals = allGoals.filter(g => g.id !== goalId); renderGoals(); }
}
